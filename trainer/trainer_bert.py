...
class trainer():
    def train(self, model, dataloader, epoch, epochs, writer, criterion, optimizer, device, max_grad_norm, scheduler=None, grad_clip=False):
        ...
        for iter, batch in enumerate(progress_bar):
            b_input_ids, b_attention_mask, b_labels = batch['input_ids'].to(device), batch['attention_masks'].to(device), batch['targets'].to(device)
            optimizer.zero_grad()
            outputs = model(
            ...
            progress_bar.set_description(f"TRAIN | Epoch: {epoch+1}/{epochs} | Loss: {(total_loss/total_samples):.4f}")
        writer.add_scalar('Train/Loss', total_loss/total_samples, epoch+1)
    
    def validation(self, model, dataloader, criterion, device, epoch, epochs):
        model.eval()
        ...
        for batch in progress_bar:
            b_input_ids, b_attention_mask, labels = batch['input_ids'].to(device), batch['attention_masks'].to(device), batch['targets'].to(device)
            with torch.no_grad():
                outputs = model(
                    input_ids=b_input_ids,
                    attention_mask=b_attention_mask
            ...
            nb_eval_steps += 1
            progress_bar.set_description(f"VAL | Epoch: {epoch+1}/{epochs} | Loss: {(total_loss/total_samples):.4f}")
