...
def clean_text_and_preserve_emojis(text):
    ...
    cleaned_text = re.sub(r'[^a-zA-ZáàảãạăắằẳẵặâấầẩẫậéèẻẽẹêếềểễệóòỏõọôốồổỗộơớờởỡợíìỉĩịúùủũụưứừửữựýỳỷỹỵĐđ\s]+', ' ', text_without_emoji)
...
abb_dict = pd.read_excel('preprocessing/normal.xlsx').set_index('abbreviation')['meaning'].to_dict()
character2emoji = pd.read_excel('preprocessing/character2emoji.xlsx').set_index('Character')['Emoji'].to_dict()
emoji2word = pd.read_excel('preprocessing/emoji2word.xlsx').set_index('Emoji')['Meaning'].to_dict()

with open("preprocessing/vn_stopwords.txt", encoding='utf-8') as file:
    stopwords = [
...
with open("preprocessing/merged_file_no_duplicates.txt", encoding='utf-8') as file:
